...
def main():
    ...
    img_path_list = sorted(os.listdir(img_dir), key=lambda x: int(x.split('.')[0].split('frame')[1]))
    ...
    for scene_name in scene_list:
        ...
        while idx != len(img_path_list):
            ...
            for cam in cams:
                ...
                # 自訂圖片讀取
                if cam == 'CAM_FRONT':
                    img_name = os.path.join(img_dir, img_path_list[idx])
                    img_ori = mmcv.imread(img_name, 'unchanged')
                    img_ori_arr.append(img_ori)
                else:
                    img_ori = np.ones_like(img_ori_arr[0]) * 128
                    img_ori_arr.append(img_ori)
            ...
            tmp_data = input_data.copy()
            tmp_data.pop('img')
            # if idx != 0 and last_metas == tmp_data:
                # print('skip')
            
            # last_metas = img_metas
