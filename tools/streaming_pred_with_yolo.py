...
# yolo + depth
cam_intrinsic = [[     1266.4,           0,      816.27],
                [          0,      1266.4,      491.51],
                [          0,           0,           1]]
config_path_depth = 'tools/mono/configs/HourglassDecoder/vit.raft5.small.py'
checkpoint_path_depth = 'ckpts/metric_depth_vit_small_800k.pth'
...
def main():
    ...
    yolo_model = YOLO('ckpts/yolo11n-seg.pt')
    ...
    for scene_name in scene_list:
        ...
        while sample['next'] != '':
            ...
            img_front = img_ori_arr[0]
            img_front = cv2.cvtColor(img_front, cv2.COLOR_BGR2RGB)
            
            rgb_origin = img_front.copy()
            
            intrinsic = [cam_intrinsic[0][0], cam_intrinsic[1][1], cam_intrinsic[0][2], cam_intrinsic[1][2]]
            
            rgb_input = transform_test_data_scalecano(rgb_origin, intrinsic, cfg.data_basic)


            yolo_result = yolo_model(rgb_origin)
            pred_depths, outputs = get_prediction(
                model=depth_model,
                input=rgb_input.unsqueeze(0),  # Stack inputs for batch processing
                cam_model=None,  
            )
